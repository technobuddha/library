import { commonOverlap } from '../string/common-overlap.ts';
 ...
 * @internal
 */
export function cleanupSemantic(diffs: Difference[]): void {
  let changes = false;
  ...
  while (pointer < diffs.length) {
    if (diffs[pointer].op === DIFFERENCE_EQUAL) {
      ...
      if (diffs[pointer].op === DIFFERENCE_INSERT) {
      ...
      if (
        ...
        // Duplicate record.
        diffs.splice(equalities.at(-1)!, 0, { op: DIFFERENCE_DELETE, text: lastEquality });
        // Change second copy to insert.
        diffs[equalities.at(-1)! + 1].op = DIFFERENCE_INSERT;
  ...
  while (pointer < diffs.length) {
    ...
    if (prev.op === DIFFERENCE_DELETE && curr.op === DIFFERENCE_INSERT) {
      ...
      if (overlapLength1 >= overlapLength2) {
        if (overlapLength1 >= dLen || overlapLength1 >= iLen) {
          ...
          diffs.splice(pointer, 0, {
            op: DIFFERENCE_EQUAL,
            text: insertion.slice(0, overlapLength1),
          });
        ...
        diffs[pointer - 1].op = DIFFERENCE_INSERT;
        diffs[pointer - 1].text = insertion.slice(0, insertion.length - overlapLength2);
        diffs[pointer + 1].op = DIFFERENCE_DELETE;
