...
export function unescapeJava(input: string): string {
  return input.replaceAll(
    ...
    /\\(([bfnrt"'\\])|([0-7]{1,3})|(u[0-9a-fA-F]{4}))/gu,
    (escape) => {
      ...
      if (c === 'r') return '\r';
      if (c === 't') return '\t';
      if (c >= '0' && c <= '7') return String.fromCodePoint(Number.parseInt(escape.slice(1), 8));
      if (c === 'u') return String.fromCodePoint(Number.parseInt(escape.slice(2), 16));
