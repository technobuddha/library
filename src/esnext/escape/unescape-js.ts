 ...
 * | \\'                  | Single Quote       | 0x27                 |
 * | \\\\                 | Backslash          | 0x5c                 |
 * | \\·…·[^1]            | Octal Escape       | 0x0000-0x01ff    |
 * | \\x··                | Hexadecimal Escape | 0x0000-0x00ff    |
 * | \\u····              | Unicode Escape     | 0x00000-0x00ffff   |
 ...
 * @category JavaScript
 */
export function unescapeJS(input: StringLike): string {
  return toString(input).replaceAll(
    // cspell:disable-next-line
    /\\(([bfnrtv"'\\])|([0-7]{1,3})|(x[0-9a-fA-F]{2})|(u[0-9a-fA-F]{4})|(u\{[0-9a-fA-F]{1,}\})|.)/gv,
    (escape) => {
