 ...
 * @category Formatting
 */
export function formatNumber(input: NumberLike, mask: string): string {
  const value = toNumber(input);

  // cspell:disable-next-line
  if (/^([CDEFGNPX][0-9]*)|R$/iv.test(mask)) {
    ...
    switch (f) {
      ...
      case 'c': {
        ...
        return format(value, { round: prec, lead: 1 })
          .minus('($', '$')
          .grouped()
          .decimal()
          .fraction()
      ...
      case 'd': {
        ...
        return format(value, { round: 0, lead: prec }).minus('-').whole().build();
      }
      case 'E':
      case 'e': {
        prec = Number.isNaN(prec) ? 6 : prec;

        return format(value, { precision: prec + 1 })
          .minus('-')
          .scientific(f)
          .build();
      }
      ...
      case 'f': {
        prec = Number.isNaN(prec) ? 2 : prec;
        return format(value, { round: prec }).minus('-').whole().decimal().fraction().build();
      }
      case 'G':
      case 'g': {
        prec = Number.isNaN(prec) ? 15 : prec;

        const sci = format(value, { precision: prec, trim: 'all' })
          .minus('-')
          .scientific(f === 'G' ? 'E' : 'e')
          .build();
        const fix = format(value, { precision: prec, trim: 'back' })
          .minus('-')
          .whole()
          .decimal()
          .fraction()
      ...
      case 'n': {
        ...
        return format(value, { round: prec }).minus('-').grouped().decimal().fraction().build();
      }
      case 'P':
      case 'p': {
        prec = Number.isNaN(prec) ? 2 : prec;

        return format(value, { scale: 2, round: prec })
          .minus('-')
          .whole()
          .decimal()
          .fraction()
  ...
  if (fmt.exponent > 0) {
    ...
    w = Math.abs(value * fmt.scale)
      .toFixed(0)
      .split(empty);
    f = [];
  } else {
    let scaled = Math.abs(value * fmt.scale);
    let rescale = 0;
    let str: string;
    let split: string[];
