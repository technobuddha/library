...
export function doubleMetaphone(value: string): [string, string] {
  ...
  if (isSlavoGermanic) {
    stdTrace.push('Slavo-Germanic');
  }
  if (isGermanic) {
    stdTrace.push('Germanic');
  }
  ...
  while (index < length) {
    ...
    switch (characters[index]) {
      ...
      case 'C':
        ...
        // ) {
        //   stdTrace.push(`${getCurrentLine().line} | ${normalized.slice(index)}: "${primary}" "${secondary}"`); index++;
        // }
      ...
      case 'J':
        ...
        if (normalized.slice(index, index + 4) === 'JOSE' || normalized.slice(0, 4) === 'SAN ') {
          if (normalized.slice(0, 4) === 'SAN ' || (index === 0 && characters[index + 4] === ' ')) {
            primary += 'H';
            secondary += 'H';
          } else {
            primary += 'J';
