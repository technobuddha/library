...
describe('relativeTime', () => {
  ...
  test('should handle the distant future or past', () => {
    const now = new Date();
  ...
  test('should handle compound date', () => {
    const now = new Date();

    expect(relativeTime(addTime(now, { minutes: 3, seconds: 30 }), now)).toBe(
      '3 minutes 30 seconds from now',
    );
    expect(relativeTime(addTime(now, { minutes: 4, seconds: 30 }), now)).toBe('4 minutes from now');
    expect(relativeTime(addTime(now, { hours: 3, minutes: 30 }), now)).toBe(
      '3 hours 30 minutes from now',
    );
    expect(relativeTime(addTime(now, { hours: 4, minutes: 30 }), now)).toBe('4 hours from now');
    expect(relativeTime(addTime(now, { days: 3, hours: 12 }), now)).toBe(
      '3 days 12 hours from now',
    );
    expect(relativeTime(addTime(now, { days: 4, hours: 12 }), now)).toBe('4 days from now');
  });

  test('should handle only 2 levels', () => {
    const now = new Date();

    expect(relativeTime(addTime(now, { hours: 3, minutes: 30, seconds: 30 }), now)).toBe(
      '3 hours 30 minutes from now',
    );
    expect(relativeTime(addTime(now, { days: 3, hours: 12, minutes: 30 }), now)).toBe(
      '3 days 12 hours from now',
    );
  });

  test('should handle today,tomorrow and yesterday', () => {
    const now = new Date();
    ...
    expect(relativeTime(now, now, { todayTomorrowYesterday: true })).toBe(
      `today ${formatDate(now, 'H:mm TT')} - now`,
    );
    expect(relativeTime(f1, now, { todayTomorrowYesterday: true })).toBe(
      `tomorrow ${formatDate(now, 'H:mm TT')} - 1 day from now`,
    );
    expect(relativeTime(p1, now, { todayTomorrowYesterday: true })).toBe(
      `yesterday ${formatDate(now, 'H:mm TT')} - 1 day ago`,
    );
