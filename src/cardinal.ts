...
export function cardinal(
  input: number,
  { groups = Infinity, digits = false, ...options }: Options = {},
): string {
...
function ordinal1000(input: number, { and, hyphen = space }: OptionsIllion): string[] {
  ...
  if (input > ZERO) {
    if (input < TWENTY) words.push(ones[input]);
    else if (input % TEN === ZERO) words.push(tens[Math.floor(input / TEN) - 2]);
    else words.push(tens[Math.floor(input / TEN) - 2] + hyphen + ones[input % TEN]);
  }
