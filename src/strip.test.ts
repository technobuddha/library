// cspell:ignore mfoo mbar mbaz mqux mred foobarbaz  foobarbazqux
...
describe('strip', () => {
  test('removes OSC escape sequences', () => {
    ...
    expect(strip(input)).toBe('foobar');

    // OSC: ESC ] ... ESC \
    const input2 = 'foo\u001B]0;title\u001B\\bar';
    expect(strip(input2)).toBe('foobar');

    // OSC: ESC ] ... ST (0x9c)
    const input3 = 'foo\u001B]0;title\u009cbar';
    expect(strip(input3)).toBe('foobar');
  });
  ...
  test('does not remove non-ANSI Unicode or control characters', () => {
    ...
    expect(strip(input)).toBe('foo\u2022bar\u0009baz');
  });
});
