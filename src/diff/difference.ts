...
import { ticksPerSecond } from '../date.ts';
...
/**
 * Indicates deleted content in a {@link Diff}
 * @group String
 * @category Difference
 */
...
/**
 * Indicates inserted content in a {@link Diff}
 * @group String
 * @category Difference
 */
...
/**
 * Indicates unmodified content in a {@link Diff}
 * @group String
 * @category Difference
 */
...
export type Diff = {
  ...
  op: typeof DIFF_DELETE | typeof DIFF_INSERT | typeof DIFF_EQUAL;
...
export type DiffInternal = Required<DiffOptions> & { deadline: number };
...
export function difference(
  text1: string,
  text2: string,
  { timeout = 0, editCost = 4, checkLines = true }: DiffOptions = {},
): Diff[] {
 ...
 * @internal
 */
function diff(text1: string, text2: string, options: DiffInternal): Diff[] {
  let str1 = text1;
  let str2 = text2;

  if (str1 === str2) {
    return str1 === empty ? [] : [{ op: DIFF_EQUAL, text: str1 }];
  }
  ...
  if (prefix.length > 0) {
    diffs.unshift({ op: DIFF_EQUAL, text: prefix });
  }
  if (suffix.length > 0) {
    diffs.push({ op: DIFF_EQUAL, text: suffix });
  }
 ...
 * @internal
 */
function compute(text1: string, text2: string, options: DiffInternal): Diff[] {
  if (text1 === empty) {
    // Just add some text (speedup).
    return [{ op: DIFF_INSERT, text: text2 }];
  }

  if (text2 === empty) {
    // Just delete some text (speedup).
    return [{ op: DIFF_DELETE, text: text1 }];
  }
  ...
  if (text1.length > text2.length) {
    ...
    if (index === -1) {
      if (text2.length === 1) {
        return [
          { op: DIFF_DELETE, text: text1 },
          { op: DIFF_INSERT, text: text2 },
        ];
      }
    } else {
      return [
        { op: DIFF_DELETE, text: text1.slice(0, index) },
        { op: DIFF_EQUAL, text: text2 },
        { op: DIFF_DELETE, text: text1.slice(index + text2.length) },
      ];
    }
  } else {
    ...
    if (index === -1) {
      ...
      return [
        { op: DIFF_INSERT, text: text2.slice(0, index) },
        { op: DIFF_EQUAL, text: text1 },
        { op: DIFF_INSERT, text: text2.slice(index + text1.length) },
      ];
    }
  }
  ...
  if (hm) {
    ...
    // Merge the results.
    return diffsA.concat([{ op: DIFF_EQUAL, text: midCommon }], diffsB);
  }
 ...
 * @internal
 */
function lineMode(text1: string, text2: string, options: DiffInternal): Diff[] {
 ...
 * @internal
 */
function bisect(text1: string, text2: string, options: DiffInternal): Diff[] {
