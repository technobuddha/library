...
export function formatDate(input: Date, mask?: string, { UTC = false }: Options = {}): string {
  ...
  return mask.replace(tokenizer, (token) => {
    switch (token) {
      case 'h':
        return padNumber(ho, 0); //Hours (24)
      case 'hh':
        return padNumber(ho, 2); //Hours (24)
      case 'H':
        return padNumber(ho % 12 || 12, 0); //Hours (12)
      case 'HH':
        return padNumber(ho % 12 || 12, 2); //Hours (12)
      case 'm':
        return padNumber(mi, 0); //Minutes
      case 'mm':
        return padNumber(mi, 2); //Minutes
      case 's':
        return padNumber(se, 0); //Seconds
      case 'ss':
        return padNumber(se, 2); //Seconds
      case 'sss':
        return padNumber(ho * secondsPerHour + mi * secondsPerMinute + se, 0); //Seconds
      case 'f':
        return padNumber(ms, 0); //Milliseconds
      case 'ff':
        return padNumber(ms, 3); //Milliseconds
      case 'YYYY':
        return padNumber(yr < 1 ? -yr + 1 : yr, 4); //Year
      case 'YY':
        return padNumber((yr < 1 ? -yr + 1 : yr) % 100, 2); //Year
      case 'M':
        return padNumber(mo + 1, 0); //Month
      case 'MM':
        return padNumber(mo + 1, 2); //Month
      case 'MMM':
        return monthAbbrev[mo]; //Month
      case 'MMMM':
        return monthName[mo]; //Month
      case 'D':
        return padNumber(da, 0); //Day
      case 'DD':
        return padNumber(da, 2); //Day
      case 'TH':
        return ordinal(da);
      case 'd':
        return dayOne[dy]; //WeekDay
      case 'dd':
        return dayTwo[dy];
      case 'ddd':
        return dayAbbrev[dy]; //WeekDay
      case 'dddd':
        return dayName[dy];
      case 'O':
        return padNumber(getDayOfYear(input, { UTC }), 0); //Day of Year (1-366)
      case 'OO':
        return padNumber(getDayOfYear(input, { UTC }), 3); //Day of Year (1-366)
      case 'Wy':
        return padNumber(getWeekOfYear(input, { UTC }).year, 0);
      case 'Ww':
        return padNumber(getWeekOfYear(input, { UTC }).week, 0); //Week of Year (1-53)
      case 'Www':
      ...
      case 'PM':
        return ho < 12 ? '' : 'PM'; //--  / PM
      case 'T':
        return ho < 12 ? 'A' : 'P'; //A   / P
      case 'TT':
        return ho < 12 ? 'AM' : 'PM'; //AM  / PM
      case 'AD':
        return yr < 1 ? '' : 'AD'; //--  / AD
      case 'BC':
        return yr < 1 ? 'BC' : ''; //BC  / --
      ...
      case 'BCE':
        return yr < 1 ? 'BCE' : ''; //BCE / --
      case 'EE':
        return yr < 1 ? 'BC' : 'AD'; //BC  / AD
      case 'EEE':
        return yr < 1 ? 'BCE' : 'CE'; //BCE / CE
      case 'J':
