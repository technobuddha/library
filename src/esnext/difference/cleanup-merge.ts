 ...
 */
export function cleanupMerge(diffs: Difference[]): void {
      ...
      case DIFFERENCE_EQUAL: {
        ...
        if (countDelete + countInsert > 1) {
          ...
          if (textDelete.length > 0) {
            diffs.splice(pointer, 0, { op: DIFFERENCE_DELETE, text: textDelete });
            pointer++;
          }
          if (textInsert.length > 0) {
            diffs.splice(pointer, 0, { op: DIFFERENCE_INSERT, text: textInsert });
            pointer++;
          }
          pointer++;
        } else if (pointer !== 0 && diffs[pointer - 1].op === DIFFERENCE_EQUAL) {
  ...
  while (pointer < diffs.length - 1) {
    if (diffs[pointer - 1].op === DIFFERENCE_EQUAL && diffs[pointer + 1].op === DIFFERENCE_EQUAL) {
