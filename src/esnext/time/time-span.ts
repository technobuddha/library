...
export class TimeSpan {
    ...
    switch (args.length) {
      ...
      case 1: {
        if (isString(args[0])) {
          ...
          const matches = /^(\d{1,2})(?::(\d\d)(?::(\d\d)(?::(\d\d))?)?)?(?:\.(\d{1,3}))?$/v.exec(
            text,
          );
          if (matches) {
            d = Number(matches[1]);
  ...
  public get days(): number {
    return Math.sign(this.clicks) * floor(Math.abs(this.clicks) / ticksPerDay);
  }
  ...
  public get hours(): number {
    return (Math.sign(this.clicks) * floor(Math.abs(this.clicks) / ticksPerHour)) % hoursPerDay;
  }
  ...
  public get minutes(): number {
    return (
      (Math.sign(this.clicks) * floor(Math.abs(this.clicks) / ticksPerMinute)) % minutesPerHour
    );
  }
  ...
  public get seconds(): number {
    return (
      (Math.sign(this.clicks) * floor(Math.abs(this.clicks) / ticksPerSecond)) % secondsPerMinute
    );
  }
  ...
  public get milliseconds(): number {
    return (Math.sign(this.clicks) * floor(Math.abs(this.clicks))) % ticksPerSecond;
  }
