...
import {
  ...
  largestInscribedRotatedRectangle,
} from '../largest-inscribed-rectangle.ts';
...
describe('largestInscribedRectangle', () => {
  ...
  test('returns the polygon itself for rectangle polygons', () => {
    ...
    // Aligned mode
    const alignedRect = largestInscribedRectangle(poly);
    expect(alignedRect).toBeDeepCloseTo({ x: 1, y: 2, width: 5, height: 3 });

    // Rotated mode should prefer 0° orientation
    const rotatedRect = largestInscribedRotatedRectangle(poly);
  ...
  test('finds correct rectangle in right triangle', () => {
    ...
    const rect = largestInscribedRectangle(triangle);
  ...
  test('handles degenerate polygons gracefully', () => {
    ...
    const rect = largestInscribedRectangle(poly);
  ...
  test('squareOnly mode produces squares', () => {
    ...
    const rectResult = largestInscribedRectangle(poly, { squareOnly: false });
    const squareResult = largestInscribedRectangle(poly, { squareOnly: true });
  ...
  test('rotated mode finds better solutions than aligned', () => {
    ...
    const alignedRect = largestInscribedRectangle(diamond);
    const rotatedRect = largestInscribedRotatedRectangle(diamond);
  ...
  test('algorithm consistency and properties', () => {
    ...
    const rectResult = largestInscribedRotatedRectangle(triangle);
    const squareResult = largestInscribedRotatedRectangle(triangle, { squareOnly: true });
  ...
  test('regression test - algorithm output stability', () => {
    ...
    const rect = largestInscribedRotatedRectangle(pentagon);
