    ...
    public pixelCount() {
        if (this.cachedCount === null) {
            ...
            for (let r = this.min.r; r <= this.max.r; r++) {
                for (let g = this.min.g; g <= this.max.g; g++) {
                    for (let b = this.min.b; b <= this.max.b; b++) {
...
class ColorMap {
    ...
    private data: PriorityQueue<ColorData>;
...
function medianCut(histo: Histogram, vbox: VBox) {
    ...
    if (maxw == deltar) {
        for (let r = vbox.min.r; r <= vbox.max.r; r++) {
            for (let g = vbox.min.g; g <= vbox.max.g; g++) {
                for (let b = vbox.min.b; b <= vbox.max.b; b++) {
