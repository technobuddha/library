 ...
 * @internal
 */
export function halfMatch(text1: string, text2: string, options: DiffInternal): string[] | null {
  ...
  const hm1 = halfMatchI(long, short, Math.ceil(long.length / 4));
  // Check again based on the third quarter.
  const hm2 = halfMatchI(long, short, Math.ceil(long.length / 2));
  let hm: string[] | null;
  if (!hm1 && !hm2) {
    return null;
  } else if (!hm2) {
