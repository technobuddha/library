 ...
 *   Stem for `value`.
 */
export function lancasterStemmer(value: string, options?: Options) {
  const settings = options || {};
  const style = settings.style;

  return applyRules(String(value).toLowerCase(), true, style === 'paper' ? rulesPaper : rulesC);
}
 ...
 * @returns {string}
 */
function applyRules(value: string, isIntact: boolean, rules: RuleCollection) {
