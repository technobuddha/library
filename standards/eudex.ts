 ...
 * Maps.
 */
const binary = (str: string) => (typeof str === 'string' ? parseInt(str, 2) : str),
  charCode = (char: string) => char.charCodeAt(0),
  charArray = (str: string) => str.split('').map(charCode);
...
export function eudex(word: string): bigint {
  if (word === '') {
    return 0n;
  }
  const array = charArray(word);

  let entry = array.length > 0 ? ((array[0] | 32) - A) & 0xff : 0;

  let firstByte = Long.UZERO;

  if (entry < LETTERS) firstByte = new Long(INJECTIVE_PHONES[entry]);
  else if (entry >= 0x7e && entry < 0x9e) firstByte = new Long(INJECTIVE_PHONES_C1[entry - 0x7e]);
  ...
  while (n < 8 && b < array.length) {
    ...
    // [Technobuddha] This causes all letters in the latin-1 range to be skipped
    // if (entry <= Z) {
    let x;

    if (entry < LETTERS) x = PHONES[entry];
    else if (entry >= 0x7e && entry <= 0x9e) x = PHONES_C1[entry - 0x7e];
    else {
      b++;
      continue;
    }
  ...
  res = res.or(firstByte.shiftLeft(56));
  return res.toBigInt();
}
