...
import { empty, negativeSign, space } from '../unicode/unicode.ts';
 ...
 * @category Formatting
 */
export type TextualizationOptions = {
  /**
 ...
 * @internal
 */
export function textualization(input: NumberLike, options: TextualizationOptions): string {
    ...
    return (
      ...
      : output.integer === 'numeric' ?
        num < 0 ?
          `${negativeSign}∞`
        : '∞'
      : num < 0 ? 'negative infinity'
      : 'infinity'
    );
  }

  const { sign, whole, fractional } = deconstructNumber(num, precision);
