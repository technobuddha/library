	...
	() => {
		...
		it(
			...
			() => {
				...
				expect(cardinal(Number.MAX_SAFE_INTEGER)).to.equal('nine quadrillion seven trillion one hundred ninety nine billion two hundred fifty four million seven hundred forty thousand nine hundred ninety one');
				expect(cardinal(Number.MAX_VALUE)).to.equal('one hundred seventy nine uncentillion seven hundred sixty nine centillion three hundred thirteen novenongintillion four hundred eighty six octonongintillion two hundred thirty one septenongintillion six senongintillion');
				//Avogadro's Number
