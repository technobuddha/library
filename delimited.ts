...
/**
  ...
  */
export function delimited(input: string, delimiter: string, index: number = 0, count: number = 1):	string
{
	if(count <= 0)
		return empty;
	else
	{
		let splits = input.split(delimiter);
		let start  = index < 0 ? splits.length + index : index;
