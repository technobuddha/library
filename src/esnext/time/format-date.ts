...
import { pad } from '../number/pad.ts';
import { ordinal } from '../textualization/ordinal.ts';
...
const tokenizer =
  /[hHmDfO]{1,2}|[s]{1,3}|YYYY|YY|[Md]{1,4}|W(y|w{1,2}|d)|TZ|GMT|TH|T{1,2}|AM|PM|CE|BCE|AD|BC|E{2,3}|J|Q|"[^"]*"|'[^']*'/gv;
...
export function formatDate(
  ...
  return argMask.replaceAll(tokenizer, (token) => {
    switch (token) {
      ...
      case 'J': {
        return pad(floor(julian(input)), 0);
      }
      case 'Q': {
        return pad(floor((mo + 3) / 3), 0);
      }
