...
import { type BinaryLike } from '../blob/binary-like.ts';
import { encodeBinary } from '../blob/encode-binary.ts';
import { toBinary } from '../blob/to-binary.ts';
...
// The CrcTable can be generated with the following code:
// for (let n = 0; n < 256; ++n) {
//   let c = n;
//   for (let k = 0; k < 8; ++k) {
...
export class Crc32 extends HashBase {
  ...
  public constructor() {
    super();
    this.crc = new U32(0xffffffff);
  }

  public update(data: BinaryLike): this;
  public update(data: string, encoding?: TextEncoding): this;
  public update(data: BinaryLike | string, encoding: TextEncoding = 'utf-8'): this {
    const buffer = typeof data === 'string' ? encodeText(data, encoding) : toBinary(data);

    for (const char of buffer) {
      this.crc = this.crc.shr(8).xor(crcTable[this.crc.xor(char).and(0xff).value]);
    }
  ...
  public digest(encoding?: BinaryEncoding): Uint8Array | string {
    const crc = this.crc.xor(0xffffffff).value;
    ...
    return encoding ? encodeBinary(buffer, encoding) : buffer;
  }
}
